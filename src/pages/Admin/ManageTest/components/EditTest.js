...
const EditTest = () => {
  ...
  const onEditTest = () => {
    ...
    const { _id, ...newTestDetail } = testDetail;
    ...
    dispatch(actions.updateTestRequest(newTestData));
  };
  ...
  useEffect(() => {
    if (status === 200) {
      alert("Sửa bài kiểm tra mới thành công!");
  ...
  useEffect(() => {
    if (error) {
      alert("Sửa bài kiểm tra mới thất bại!");
      window.location.reload();
    }
  }, [error]);
    ...
    <Box className={classes.createTestBody}>
      <Box className={classes.testField}>
        ...
        <Box className={classes.saveBtnField}>
          <Button className={classes.saveBtn} onClick={onEditTest}>
            Lưu bài kiểm tra
          </Button>
        </Box>
      </Box>
